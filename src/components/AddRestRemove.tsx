...
export default function AddRestRemove({handleIncrement,handleDecrement, onRemove, quantity}: actionsInterfaces){
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-100 px-2 py-1 rounded-md shadow-sm">
      {/* Quantity controls */}
      <button onClick={handleDecrement} className="px-2 py-1 bg-gray-400 rounded hover:bg-gray-600 text-sm font-bold">−</button>
      <span className="text-xl text-black font-bold">{quantity}</span>
      <button onClick={handleIncrement} className="px-2 py-1 bg-gray-400 rounded hover:bg-gray-600 text-sm font-bold">+</button>

      {/* Add to cart button */}
      <button onClick={onRemove} className="flex items-center gap-1 px-2 py-1 bg-red-600 text-white text-base font-semibold rounded-md hover:bg-red-700 transition duration-300 shadow-sm">
